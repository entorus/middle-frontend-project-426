...
`/checkout` пока показывает проверку состава корзины, без создания или отправки заказа.
С пустой корзиной прямой переход возвращает на `/cart`. Серверный расчёт и создание заказа — следующий шаг.
