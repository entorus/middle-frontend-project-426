...
Интерфейс оформлен на Tailwind CSS 4 через официальный Vite-плагин `@tailwindcss/vite`.
В `apps/front/src/style.css` находится только импорт Tailwind: собственных CSS-правил,
`@apply` и inline-стилей нет. Utility-классы задаются в компонентах; общие кнопки, поля
и варианты ссылок находятся в `shared/ui.tsx`, индикатор наличия — в `catalog/Availability.tsx`.
Адаптивная раскладка использует стандартные брейкпоинты Tailwind.
Визуальный ориентир — [скриншоты проекта Хекслета](https://files.hexlet.app/a/qf7bsq).

### Модули

Фронтенд в `apps/front/src` разделён по функциям магазина:

- `auth/` — контекст пользователя, запросы авторизации, форма, навигация и защита страниц.
- `catalog/` — страница каталога, фильтры, выдача, карточка и страница товара. `useCatalogFilters` управляет URL и задержкой поиска.
- `cart/` — хранение идентификаторов и количества, контекст корзины, загрузка свежих товаров и компоненты позиций.
- `orders/` — оформление, подтверждение и история. `useCheckout` управляет отправкой; `CheckoutForm`, `OrderError` и `OrderDetails` отвечают за отображение.
- `home/` — промо-компоненты главной; `shared/money.ts` — единое форматирование денежных сумм.

Файлы `index.ts` экспортируют публичные компоненты и хуки модуля. Внутри модуля
используются прямые импорты соседних файлов. Типы API берутся из `generated/`,
а не описываются повторно вручную.

На бэкенде `apps/api/src/auth.ts`, `catalog.ts` и `orders.ts` регистрируют HTTP-маршруты.
`auth/session.ts` отвечает за cookie и серверные сессии, `auth/validateOrigin.ts` — за проверку источника запроса.
`catalog/products.ts` содержит общий запрос товаров и преобразование строк БД.
`orders/createOrder.ts` сохраняет заказ атомарно, а `orders/repository.ts` собирает ответы из сохранённых снимков.
Миграции, сиды и сгенерированный контракт остаются в своих каталогах.
